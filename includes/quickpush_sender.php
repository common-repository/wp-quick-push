...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    $quickpush_title = ($_POST['quickpush_push_title']) ? stripslashes(sanitize_text_field($_POST['quickpush_push_title'])) : false;
    $quickpush_content = ($_POST['quickpush_push_content']) ? stripslashes(sanitize_text_field($_POST['quickpush_push_content'])) : false;
    $quickpush_url = ($_POST['quickpush_push_url']) ? sanitize_text_field($_POST['quickpush_push_url']) : false;
    if (get_option('quickpush_appID') == null || get_option('quickpush_restApi') == null) { ?>
    <div class="error"><p><strong><?php _e('Incomplete details! Kindly check OneSignal.com Account settings and try again.' ); ?></strong></p></div>
    <?php } elseif(!$quickpush_title || empty($quickpush_title)){ ?>
    ...
    <?php } else {
            ...
            if (get_option('quickpush_everyone') != 'true'){
                $segments = explode(",", get_option('quickpush_pushSegments'));
                // $segmentArray "[\"".implode('", "', $segments)."\"]";
            } else{
                $segments = array('All');
            }
            ...
            if(empty($quickpush_content) && empty($quickpush_url)){
                $push_status_json = quickpushme(get_option('quickpush_appID'), get_option('quickpush_restApi'), $json, $segments, false);
            } else {
                $push_status_json = quickpushme(get_option('quickpush_appID'), get_option('quickpush_restApi'), $json, $segments, true);
            }
            $push_status = json_decode($push_status_json, true);
            if($push_status['id']){ ?>
    <div class="updated"><p><strong><?php _e('Push sent successfully to '.$push_status['recipients'].' recipients.'); ?></strong></p></div>            
    <?php } else { ?>
                            ...
                            <form name="sendPush_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
                                <table class="form-table">
                                    ...
                                    <tr valign="top">
                                        ...
                                        <td>
                                            <p class='description'><?php echo (get_option('quickpush_everyone')=='true') ? _e('Everyone') : _e('Segments').": ". get_option('quickpush_pushSegments'); ?></p>
                                        </td>
                                    </tr>                                   
                                </table>                                
                                <p class="submit">  
