...
//Form data sent 
    ...
    $JoeNoChannel = '';
    if (isset($_POST['quickpush_everyone'])) {  
        update_option('quickpush_everyone', 'true');
        $JoeNoChannel = ' checked="checked"';
    }
    else
        update_option('quickpush_everyone', 'false');

    $JoePushChannels = trim(sanitize_text_field($_POST['quickpush_pushSegments']), " ");
    update_option('quickpush_pushSegments', $JoePushChannels); ?>  
...
<?php
    ...
    $JoePushChannels = get_option('quickpush_pushSegments');

    $JoeNoChannel = '';
    if (get_option('quickpush_everyone') == 'true') 
        $JoeNoChannel = ' checked="checked"';
} ?>
<div class="wrap">    
                    ...
                    <div class="postbox">                    
                        <div class="inside">
                            <form name="quickpush_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
                                <h3>
                                    <span><?php    echo __( 'OneSignal.com Push Service - Settings', 'quickpush_trdom' ) . " (OneSignal.com <a href=\"http://onesignal.com/apps\" target=\"_blank\">Dashboard</a>)"; ?>  </span>                                    
                                </h3>                        
                                ...
                                <table class="form-table">
                                    ...
                                    <tr valign="top">
                                        ...
                                        <td><input type="text" name="quickpush_appID" placeholder="Paste the Application ID from the OneSignal.com Dashboard > App Settings > Keys" value="<?php echo $JoeAppID; ?>" style="width:90%;"></td>
                                    </tr>
                                    <tr valign="top">
                                        <td scope="row"><label for="tablecell"><i><?php _e("REST API Key: " ); ?></i></label></td>
                                        <td><input type="text" name="quickpush_restApi" placeholder="Paste the REST API Key from the OneSignal.com Dashboard > App Settings > Keys" value="<?php echo $JoeRestApi; ?>" style="width:90%;"></td>
                                    </tr>
                                    <tr valign="top">
                                ...
                                <table class="form-table">
                                    <tr valign="top">
                                        <td scope="row"><label for="tablecell">Segments</label></td>
                                        <td><input type="text" name="quickpush_pushSegments" placeholder="e.g. Active Users,Engaged Users,Inactive Users" value="<?php echo $JoePushChannels; ?>" class="regular-text">
                                            <p class="description"><strong>Comma</strong> separated (Space Counted) names for segments to be receiving the notifications. If not specified, then by default Global Broadcast Channel (GBC) shall be selected.</p>
                                        </td>
                                    </tr>
                                    <tr valign="top" >
                                        <td scope="row"><label for="tablecell"></label></td>
                                        <td><input type="checkbox" name="quickpush_everyone" <?php echo $JoeNoChannel; ?> > Do not include segments, just send notifications to all.</td>
                                    </tr>
                                </table>
